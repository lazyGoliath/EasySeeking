...
function SummaryForm({ enabledNext }: SummaryFormProps) {
    const { resumeInfo, setResumeInfo } = useContext(ResumeInfoContext);
    const [summary, setSummary] = useState<string>("");
    const [loading, setLoading] = useState(false);
    const params = useParams();
    const [aiGeneratedSummaryList, setAiGenerateSummaryList] = useState<Array<{ experience_level: string; summary: string }>>([]);

    useEffect(() => {
        if (summary && setResumeInfo && resumeInfo) {
            setResumeInfo({
                ...resumeInfo,
                summary: summary
            });
